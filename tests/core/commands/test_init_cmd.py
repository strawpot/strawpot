...
class TestRunInit:
    def test_creates_strawpot_directory(self, tmp_path: Path):
        run_init(tmp_path)
        assert (tmp_path / ".strawpot").is_dir()

    def test_creates_subdirectories(self, tmp_path: Path):
        run_init(tmp_path)
        lt = tmp_path / ".strawpot"
    ...
    def test_creates_project_yaml(self, tmp_path: Path):
        run_init(tmp_path)
        project_yaml = tmp_path / ".strawpot" / "project.yaml"
    ...
    def test_creates_gitignore_entry(self, tmp_path: Path):
        ...
        assert gi.exists()
        assert ".strawpot/runtime/" in gi.read_text()
    ...
    def test_appends_to_existing_gitignore(self, tmp_path: Path):
        ...
        assert ".strawpot/runtime/" in content

    def test_does_not_duplicate_gitignore_entry(self, tmp_path: Path):
        gi = tmp_path / ".gitignore"
        gi.write_text(".strawpot/runtime/\n")
        run_init(tmp_path)
        content = gi.read_text()
        assert content.count(".strawpot/runtime/") == 1
    ...
    def test_force_adds_missing_roles(self, tmp_path: Path):
        run_init(tmp_path)
        # Delete one role
        (tmp_path / ".strawpot" / "roles" / "fixer.yaml").unlink()
        run_init(tmp_path, force=True)
        assert (tmp_path / ".strawpot" / "roles" / "fixer.yaml").exists()
