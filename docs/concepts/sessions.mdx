...
Agents in a conversation are asked to end their response with a structured `## Session Recap` section (what was accomplished, key decisions, open items). The database stores the full agent output as the session summary. When building context for the next turn, the context builder extracts just the recap section — giving subsequent sessions a focused signal while keeping the full output available for display in the GUI.
