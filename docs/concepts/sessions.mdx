...
|-------------|------------------------|
| **Created a PR** | Nothing to merge — the PR is the work product. Worktree is cleaned up, remote branch preserved. |
| **No PR** | Changes are patch-applied as **unstaged changes** to the original working directory, then worktree is cleaned up. |

### How it works

1. On session end, StrawPot checks whether the session branch has an open pull request (via `gh pr list`)
2. **If PR exists** → just clean up the worktree. The work lives on the remote.
3. **If no PR** → generate a patch (`git diff base..session`), apply it to the base directory as unstaged changes
4. If the patch has conflicts, the user is prompted for resolution (apply all, skip conflicts, or discard)
...
3. For dead sessions: detects PR status, patch-applies changes if needed, and cleans up the worktree
