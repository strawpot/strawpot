...
Agents in a conversation are asked to end their response with a structured `## Session Recap` section (what was accomplished, key decisions, open items). When present, this recap is stored as the session summary instead of the raw output, giving the next turn a more useful signal.
