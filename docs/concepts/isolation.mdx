...
| `worktree` | Create a git worktree per session | **Deprecated** — use the [worktree skill](/skills/worktree) instead |
...
isolation = "worktree"  # deprecated — use the worktree skill instead
```

```bash
strawpot start --isolation worktree  # deprecated
```
...
> **Deprecated.** Session-level worktree isolation is deprecated and will be removed in a future version. Use the [worktree skill](/skills/worktree) for agent-controlled isolation instead.
