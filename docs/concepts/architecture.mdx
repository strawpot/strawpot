...
`interrupt` sends a soft cancel to the agent. Returns `True` if the signal was explicitly forwarded (e.g. via tmux), `False` if the agent received it directly from the OS.
