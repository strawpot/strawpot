# Content Launch Results

Tracking document for the first content launch campaign.
Fill in as results come in after posting.

## Pre-Launch Baselines

Capture these **before** posting anything.

| Metric            | Value | Date captured |
|-------------------|-------|---------------|
| GitHub stars      |       |               |
| PyPI downloads (7d) |     |               |
| strawpot.com visitors (7d) | |            |

## Hacker News

- **Post URL**:
- **Title**: Show HN: I tried to automate my entire company with 32 AI agents
- **Posted at**: (weekday, 9-11am ET)
- **First comment posted**: yes / no

| Time after post | Points | Comments | Front page? |
|-----------------|--------|----------|-------------|
| +1h             |        |          |             |
| +4h             |        |          |             |
| +12h            |        |          |             |
| +24h            |        |          |             |
| +48h            |        |          |             |

- **Peak front-page position**: N/A
- **Total time on front page**: N/A

## Reddit

### r/programming

- **Post URL**:
- **Posted at**:
- **Title**:

| Time after post | Upvotes | Comments |
|-----------------|---------|----------|
| +4h             |         |          |
| +24h            |         |          |
| +48h            |         |          |

### r/artificial

- **Post URL**:
- **Posted at**:
- **Title**:

| Time after post | Upvotes | Comments |
|-----------------|---------|----------|
| +4h             |         |          |
| +24h            |         |          |
| +48h            |         |          |

### r/SideProject

- **Post URL**:
- **Posted at**:
- **Title**:

| Time after post | Upvotes | Comments |
|-----------------|---------|----------|
| +4h             |         |          |
| +24h            |         |          |
| +48h            |         |          |

## Twitter / X

- **Thread URL**:
- **Posted from**: personal account
- **Posted at**:
- **Skipped?**: yes (< 500 followers) / no

| Time after post | Impressions | Likes | Retweets | Replies |
|-----------------|-------------|-------|----------|---------|
| +4h             |             |       |          |         |
| +24h            |             |       |          |         |
| +48h            |             |       |          |         |

## Post-Launch Metrics (48h after first post)

| Metric            | Before | After | Delta |
|-------------------|--------|-------|-------|
| GitHub stars      |        |       |       |
| PyPI downloads (7d) |      |       |       |
| strawpot.com visitors (7d) | |     |       |
| New GitHub issues  | N/A   |       |       |

## Notable Responses

Track any notable comments, feedback, or conversations:

1. _None yet._

## Lessons Learned

_Fill in after the launch is complete._
