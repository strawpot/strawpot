...
# Cache delegation results within a session
# Identical (role, task, format) requests return cached results without spawning
cache_delegations = true

# Max cached delegation entries (0 = unlimited)
cache_max_entries = 0

# Max cache age in seconds (0 = unlimited)
cache_ttl_seconds = 0

# Max delegation calls per session (0 = unlimited)
# Counts both cache hits and spawned agents to prevent infinite loops
max_num_delegations = 0
...
Agent-specific environment variables (e.g. `ANTHROPIC_API_KEY`) are declared in the agent's `AGENT.md` manifest and prompted for interactively when missing. Prompted values are automatically saved to `[agents.<name>.env]` in `strawpot.toml` for future sessions.
...
## Agent Env Persistence

Agent environment variables work the same way as skill env vars. When StrawPot prompts for a missing agent env var (e.g. `ANTHROPIC_API_KEY`), the value is automatically saved to `[agents.<name>.env]` in the global `strawpot.toml`.

```toml
[agents."strawpot-claude-code".env]
...
1. `os.environ` — An explicit shell variable always wins
2. Saved config — Values from `[agents.<name>.env]` in `strawpot.toml`
3. Interactive prompt — Only if neither source provides a value

<Note>
Both skill and agent env values are stored as plain text. Do not commit `strawpot.toml` files that contain secrets. Add `strawpot.toml` to `.gitignore` if it holds sensitive values. Future versions may support OS keychain integration.
</Note>
